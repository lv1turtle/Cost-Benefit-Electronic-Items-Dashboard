...
def get_intel_core_cpu(file, core):
    """
    ...
    processed = preprocessor.extract_rows_by_keyword_contains(keyword=core, target_column='cpu_name')
    ...
    for row in processed.itertuples():
        ...
        if end_idx != -1:
            print(cpu_name[start_idx:end_idx].strip())
            new.loc[len(new)] = cpu_name[start_idx:end_idx].strip()
        else:
            print(cpu_name[start_idx:].strip())
            new.loc[len(new)] = cpu_name[start_idx:].strip()
    return new


def get_ryzen_cpu_name(file, core):
    """
    ...
    """
    preprocessor = Preprocessor(file)
    processed = preprocessor.extract_rows_by_keyword_contains(keyword=f"Ryzen {core}", target_column='cpu_name')
    new = pd.DataFrame(columns=['cpu_name'])
    for row in processed.itertuples():
        cpu_name = row.cpu_name
        repeated = 'AMD'
...
def add_keyword_with_cpu_name(file, company):
    """
    :param file: intel, ryzen cpu name file:
